...
class MozillaInteraction:
    ...
    def load_profile(self, profile):
        ...
        return self.proxy.initialize(self.profile)
...
def ask_section(sections: ConfigParser):
    ...
    choice = "ASK"
    for sec in sections:
        final_choice.append(sections[sec])
    #print(final_choice)
...
def get_profile(basepath: str, interactive: bool, choice: Optional[str], list_profiles: bool):
    ...
    try:
        ...
        if len(sections) == 1:
            ...
            all_section = ask_section(sections)
        profile = []
        for section in all_section:
            temp = os.path.join(basepath, section)
...
def password() -> None:
    ...
    profiles = get_profile(basepath, args.interactive, args.choice, args.list)
    count = 1
    try:
        for profile in profiles:
            #print(profile)
            # Start NSS for selected profile
            flag = moz.load_profile(profile)
            ...
            if flag == False:
                ...
                #print(formatter)
                all_pass = formatter.output()
                with open('Firefox profile '+str(count)+'.txt','w') as f:
                    for website in all_pass:
                        f.write(website)
                    f.close()
                count = count + 1
    ...
    try:
        for profile in profiles:
            ...
            if flag == False:
                ...
                #print(formatter)
                all_pass = formatter.output()
                fileloc = os.path.join(temp(),'Firefox profile '+str(count)+'.txt')
